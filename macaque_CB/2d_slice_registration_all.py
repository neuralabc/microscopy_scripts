...
# all_image_fnames = [image.split('.')[0] for image in all_image_fnames] #remove the .tif extension to comply with formatting below
rescale=10
...
template = output_dir+subject+'_'+all_image_fnames[largest]+'.nii.gz'    
...
for idx,img in reversed(list(enumerate(all_image_fnames))):
        ...
        if idx<len(all_image_fnames)-1:
            next1 = output_dir+subject+'_'+all_image_fnames[idx+1]+'_coreg2nl_ants-def0.nii.gz'
            sources.append(nifti)
            targets.append(next1)
        
        # second next image
        if idx<len(all_image_fnames)-2:
            next2 = output_dir+subject+'_'+all_image_fnames[idx+2]+'_coreg2nl_ants-def0.nii.gz'
            sources.append(nifti)
            targets.append(next2)
        
        # third next image
        if idx<len(all_image_fnames)-3:
            next3 = output_dir+subject+'_'+all_image_fnames[idx+3]+'_coreg2nl_ants-def0.nii.gz'
            sources.append(nifti)
