...
for idx,img_orig in enumerate(all_image_fnames):
    ...
    output = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'.nii.gz'
...
for idx,img in enumerate(all_image_fnames):
    ...
    nifti = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'.nii.gz'
...
template = output_dir+subject+'_'+str(largest).zfill(4)+'_'+os.path.basename(all_image_fnames[largest]).split('.')[0]+'.nii.gz'    
...
for idx,img in enumerate(all_image_fnames):
    ...
    output = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg0nl.nii.gz'
...
if (os.path.isfile(template)):
    ...
    for idx,img in enumerate(all_image_fnames):
        img = os.path.basename(img).split('.')[0]
        reg = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg0nl_ants-def0.nii.gz'
        print(reg)
...
for idx,img in enumerate(all_image_fnames):
        img = os.path.basename(img).split('.')[0]
        # current image
        nifti = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg0nl_ants-def0.nii.gz'
        sources = [nifti]
        targets = [template]
        # previous image
        if idx>0:
            prev1 = output_dir+subject+'_'+str(idx-1).zfill(4)+'_'+all_image_names[idx-1]+'_coreg1nl_ants-def0.nii.gz'
            sources.append(nifti)
            targets.append(prev1)
        
        # second previous image
        if idx>1:
            prev2 = output_dir+subject+'_'+str(idx-2).zfill(4)+'_'+all_image_names[idx-2]+'_coreg1nl_ants-def0.nii.gz'
            sources.append(nifti)
            targets.append(prev2)
        
        # third previous image
        if idx>2:
            prev3 = output_dir+subject+'_'+str(idx-3).zfill(4)+'_'+all_image_names[idx-3]+'_coreg1nl_ants-def0.nii.gz'
            sources.append(nifti)
            targets.append(prev3)
        
        output = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg1nl.nii.gz'
        coreg1nl = nighres.registration.embedded_antspy_2d_multi(source_images=sources, 
                        target_images=targets,
                        run_rigid=True,
...
if (os.path.isfile(img_stack)):
    ...
    for idx,img in enumerate(all_image_fnames):
        img = os.path.basename(img).split('.')[0]
        reg = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg1nl_ants-def0.nii.gz'
        stack.append(nighres.io.load_volume(reg).get_fdata())
...
for idx,img in reversed(list(enumerate(all_image_fnames))):
        img = os.path.basename(img).split('.')[0]
        # current image
        nifti = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg1nl_ants-def0.nii.gz'
     
        sources = [nifti]
        targets = [template]
        # next image
        if idx<len(all_image_fnames)-1:
            next1 = output_dir+subject+'_'+str(idx+1).zfill(4)+'_'+all_image_names[idx+1]+'_coreg2nl_ants-def0.nii.gz'
            sources.append(nifti)
            targets.append(next1)
        
        # second next image
        if idx<len(all_image_fnames)-2:
            next2 = output_dir+subject+'_'+str(idx+2).zfill(4)+'_'+all_image_names[idx+2]+'_coreg2nl_ants-def0.nii.gz'
            sources.append(nifti)
            targets.append(next2)
        
        # third next image
        if idx<len(all_image_fnames)-3:
            next3 = output_dir+subject+'_'+str(idx+3).zfill(4)+'_'+all_image_names[idx+3]+'_coreg2nl_ants-def0.nii.gz'
            sources.append(nifti)
            targets.append(next3)
        
        output = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg2nl.nii.gz'
        coreg1nl = nighres.registration.embedded_antspy_2d_multi(source_images=sources, 
                        target_images=targets,
                        run_rigid=True,
                        rigid_iterations=1000,
...
if (os.path.isfile(img_stack)):
    ...
    for idx,img in enumerate(all_image_fnames):
        img = os.path.basename(img).split('.')[0]
        reg = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg2nl_ants-def0.nii.gz'
        stack.append(nighres.io.load_volume(reg).get_fdata())
...
for idx,img in enumerate(all_image_fnames):
    ...
    output = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg12nl_ants-def0.nii.gz'
    if (not os.path.isfile(output)):
        slice1 = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg1nl_ants-def0.nii.gz'
        slice2 = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg2nl_ants-def0.nii.gz'
        ...
        mapping= output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg12nl_ants-map.nii.gz'
        inverse= output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg12nl_ants-invmap.nii.gz'
        if (mi1c>mi2c): 
            mapping1= output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg1nl_ants-map.nii.gz'
            inverse1= output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg1nl_ants-invmap.nii.gz'
            shutil.copyfile(mapping1, mapping)
            shutil.copyfile(inverse1, inverse)
            shutil.copyfile(slice1, output)
        else:
            mapping2= output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg2nl_ants-map.nii.gz'
            inverse2= output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg2nl_ants-invmap.nii.gz'
...
if (os.path.isfile(img_stack)):
    ...
    for idx,img in enumerate(all_image_fnames):
        img = os.path.basename(img).split('.')[0]
        reg = output_dir+subject+'_'+str(idx).zfill(4)+'_'+img+'_coreg12nl_ants-def0.nii.gz'
        stack.append(nighres.io.load_volume(reg).get_fdata())
