...
auto_dMap_computation = false // turn off for PC counts use the automatic density map calculations (faster, but cannot yet control res)
manual_pixel_cell_count = true //use float processor to compute cell count image based on user specifications (see below)
save_centroids_csv = true // output centroid x,y coordinates to a csv file
...
def run_cell_counting() {
    // *************************** RUN THE PC CELL DETECTION ***************************
    runPlugin('qupath.imagej.detect.cells.WatershedCellDetection', '{"detectionImageBrightfield": "Optical density sum",  "requestedPixelSizeMicrons": 0.0,  "backgroundRadiusMicrons": 0.0,  "medianRadiusMicrons": 0.0,  "sigmaMicrons": 1.5,  "minAreaMicrons": 30.0,  "maxAreaMicrons": 500.0,  "threshold": 0.2,  "maxBackground": 2.0,  "watershedPostProcess": true,  "excludeDAB": false,  "cellExpansionMicrons": 5.0,  "includeNuclei": true,  "smoothBoundaries": true,  "makeMeasurements": true}');
    runObjectClassifier("purk_classifier");
}
...
if (getProjectEntry().getImageName().contains("20x")) { 
    ...
    if (auto_dMap_computation){
        ...
        fileName_dMap = buildFilePath(out_dir, imageName + '_PC_densityMap_'+ (int) dMap_radius + '_rad_' + (int) autoDownsampleValue + '_downsample_'+ dMap_requestedPixelSizeMicrons.round(3).toString().replace('.','p')+'um_pix.tif')
    ...
    //    def imageData = getCurrentImageData()
        ...
        fileName_manual = buildFilePath(out_dir, imageName + '_PC_cellCount_' + (int) requestedDownSampleValue + '_downsample_' + requestedPixelSizeMicrons.round(3).toString().replace('.','p')+'um_pix.tif')
    ...
    if (save_centroids_csv) {
        fileName_csv = buildFilePath(out_dir, imageName.replace('.*','') + '_PC_cellCentroidXY_' + requestedPixelSizeMicrons.round(3).toString().replace('.','p')+'um_pix.csv')
        ...
        for (detection in detections.findAll {it.getPathClass() == getPathClass('Positive')}) {
